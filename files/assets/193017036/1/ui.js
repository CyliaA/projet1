...
Ui.prototype.initializeForm = function () {
// Déclaration des variables globales


// Un autre script pour écouter les événements

document.addEventListener('assistance-confirmed', function() {
    console.log('Assistance has been confirmed in another script.');
    // Votre logique ici pour gérer la confirmation d'assistance
    // Par exemple, afficher un autre formulaire ou effectuer une action
    window.speechSynthesis.speak(utterance);
});

document.addEventListener('assistance-declined', function() {
    console.log('Assistance has been declined in another script.');
    // Votre logique ici pour gérer le refus d'assistance
    // Par exemple, afficher un message ou effectuer une autre action
        ...
        if (nextStep.type === 'group') {
            ...
            setTimeout(() => {
                ...
                if (nextStep.type === "radio") {
                 ...
                 // Afficher le champ de texte si le type est "text"
                else if (nextStep.type === "text") {
                    addTextField(nextStep.field); // Ajouter un champ de texte
                }
            }, 1000);
        }
    } else {
...
function validateEmail(email) {
    const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; // Expression régulière pour vérifier le format de l'e-mail
