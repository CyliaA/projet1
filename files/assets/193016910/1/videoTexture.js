...
VideoTexture.prototype.initialize = function() {
    ...
    // Set muted to false to allow sound
    video.muted = false;
    ...
    // Disable autoplay to comply with browser policies
    video.autoplay = false;

    // iOS video texture playback requires that you add the video to the DOM
    ...
    style.width = '80vw'; // Utilisez des unités vh/vw pour être réactif à la taille de l'écran
    style.height = '45vw'; // Ajustez ce rapport pour conserver les proportions de l'iframe
    style.border = 'none';
    ...
    // Add event listener to play video with sound on external interaction
    this.app.on('playVideo', function() {
        video.play();
    });

    // Notify when video ends
    video.addEventListener('ended', function() {
        this.app.fire('videoEnded');
    }.bind(this));
