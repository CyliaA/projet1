var VideoTexture4 = pc.createScript('videoTexture4');

VideoTexture4.attributes.add('videoAsset', {
    title: 'Video Asset',
    description: 'MP4 video asset to play back on this video texture.',
    type: 'asset'
});

VideoTexture4.attributes.add('videoUrl', {
    title: 'Video Url',
    description: 'URL to use if there is no video asset selected',
    type: 'string'
});

VideoTexture4.attributes.add('playEvent', {
    title: 'Play Event',
...
VideoTexture4.prototype.initialize = function() {
    var app = this.app;
    ...
    this.app.on('playVideo4', function(data) {
        console.log(data.message); // Affiche "Playing video 4!"
        video.play();
    });

    video.addEventListener('ended', function() {
        this.app.fire('videoEnded4');
    }.bind(this));
...
VideoTexture4.prototype.update = function(dt) {
